...
case "$cmd" in
	tag)
		...
		fail-msg "$tags" "git-fzf-branch: no tags here!"
		t=$tags
		;;
	branch)
		# [[ "$branches" == '' ]] && echo "git-fzf-branch: no branches here."; exit 1
		fail-msg "$branches" "git-fzf-branch: no branches here!"
		t=$branches
		;;
	checkout)
		fail-msg "$branches$tags" "git-fzf-branch: no branches or tags beyond the 'master'."
